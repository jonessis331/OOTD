...
const InfoSheet = ({ item }) => {
  ...
  const material =
    openAITags?.openTagsTwo?.material ||
    openAITags?.scraped_tags?.material ||
    "unknown material";

  // Join the occasions if it's an array
  const occasionArray =
    openAITags?.openTagsOne?.occasion ||
    openAITags?.openTagsTwo?.occasion ||
    openAITags?.scraped_tags?.occasion ||
    [];
  const occasion = Array.isArray(occasionArray)
    ? occasionArray.join(", ")
    : "various occasions";

  const brand =
    openAITags?.openTagsOne?.brand_name ||
    openAITags?.openTagsTwo?.["brand name"] ||
    openAITags?.scraped_tags?.brand_name ||
    "a renowned brand";
  const category =
    openAITags?.openTagsOne?.category ||
    openAITags?.openTagsTwo?.category ||
    openAITags?.scraped_tags?.category ||
    "fashion category";
  const style =
    openAITags?.openTagsOne?.style ||
    openAITags?.openTagsTwo?.style ||
    openAITags?.scraped_tags?.style ||
    "a stylish piece";

  // Get labels with high confidence (> 0.8)
  const relevantLabels =
    deepTags?.labels?.filter((label) => label.confidence > 0.8) || [];
  const labelsText = relevantLabels.map((label) => label.name).join(", ");
  const thumbnail = item?.googleItem?.thumbnail;
  const title =
    item?.googleItem?.title ||
    openAITags?.openTagsOne?.name_of_item ||
    openAITags?.openTagsTwo?.name_of_item ||
    openAITags?.scraped_tags?.title;
